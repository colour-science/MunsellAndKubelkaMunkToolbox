function MacAdamMatrix = MakeRenotationMacAdamLimitMatrix();
% Purpose		Make a matrix that gives the maximum chroma for which renotation data is
%				available, for all combinations of hues and values.
%
...
% routine MakeRenotationMatrices.
load RenotationMatrices.mat

% Initialize a matrix in Matlab format, and one to be used in
% outputting a text file.
MacAdamMatrix = NaN * ones(40,9);
outputmatrix  = [];

% The three matrices (x,y, and Y) in the structure RenotationMatrices, all
...
[row col] = size(RenotationMatrices.x);
for H = 1:40		% Loop over hues
   for V = 1:9		% Loop over values
      ...
      chromalist = [];
	  for C = 1:19
	     if ~isnan(RenotationMatrices.x(H,V,C))
		    chromalist = [chromalist 2*C];
		 end
	  end
	  MaxChroma          = max(chromalist);
	  MacAdamMatrix(H,V) = MaxChroma;
...
fid = fopen('MacAdamLimitsFromRenotationData.txt', 'w');
...
fprintf(fid, 'respect to illuminant C.  Table I of [Newhall1943] lists xyY coordinates for\n');
fprintf(fid, 'Munsell colours defined by 40 hue designations, 9 integer values, and even chromas.\n');
fprintf(fid, 'The MacAdam limit colours occur when, for a given hue and value, the chroma\n');
fprintf(fid, 'reaches a physically realizable maximum.  The data in this file lists the maximum\n');
fprintf(fid, 'chromas that appear in Table I of [Newhall1943].\n');
fprintf(fid, 'This file was generated by the routine MakeRenotationMacAdamLimitMatrix.m, which\n');
...
fprintf(fid, 'MunsellRenotation.txt, which is a modified version of the file real.dat, obtained\n');
fprintf(fid, 'from [MCSL2010].\n');
fprintf(fid, 'The data in this file is expected to be useful when inverting the Munsell renotation,\n');
fprintf(fid, 'and when determining what colours are within the MacAdam limits.  For ease of use,\n');
fprintf(fid, 'the data has also been saved in the Matlab file MaxChromasInRenotationData.mat,\n');
fprintf(fid, 'which can be loaded directly into Matlab or Octave.\n');
fprintf(fid, 'Since this file is based on renotation data, every maximum chroma listed is an even\n');
fprintf(fid, 'integer.  Another file, MaxChromasForStandardMunsellHuesAndValues.txt, based on\n');
fprintf(fid, 'extrapolations of the renotation data, gives more accurate estimates of maximum\n');
fprintf(fid, 'chroma, which are not required to be integers.\n');
fprintf(fid, '\n');
...
% Write the maximum chroma data to the file.
...
% Also, save the MacAdam limits in Matlab format, for other Matlab routines.
save MaxChromasInRenotationData.mat MacAdamMatrix
return;